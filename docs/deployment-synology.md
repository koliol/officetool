...
`Auth__Enabled` 默认 `true`。首次以默认配置启动时，应用会检测到用户表为空，
**自动创建一个本地超级管理员**，并把初始密码打印到容器日志（只打印这一次）：

```bash
docker compose logs officetool | grep 初始密码
# 已创建本地超级管理员 admin，初始密码：k7KpQmXw3nRtYb2f
```

用它登录网页 → 改密 → 到「用户组 / 授权配置」里建组并授权，其他人才看得到内容。

> 漏看了日志也不要紧：删掉 `officetool-state` 卷重来即可。
> 这只丢项目/模板的索引记录，`/data` 里的实际文件不受影响，重新同步一次就回来了。

**为什么要自动建这个账号**：权限配错的典型后果是「所有人都不再有管理权限」，
此时没有任何界面能修——只能改数据库。留一个不受 ACL 约束的超管是这个系统唯一的逃生舱。

### 7.2 三种身份通道

| 通道 | 适用 | 说明 |
|---|---|---|
| Cookie | 网页 | SSO 与表单登录最终都换发这张票，后续请求只认它 |
| Negotiate（SSO）| 已加域的内网 PC | 浏览器免登录。仅挂 `GET /api/auth/sso` |
| Bearer 令牌 | 脚本 / 自动化 | 在网页「访问令牌」页生成，带 `Authorization: Bearer` |

**桌面插件不在此列**：它只处理 `officetool://` 协议、把共享路径交给 Windows 打开，
全程不调用接口，因此无论鉴权开不开都能用。

### 7.3 接入域（按需）

只填本地账号可以不看这节。要让域用户免登录并按域组授权，需要：

```ini
# .env
AUTH_DOMAIN=contoso.com
AUTH_LDAP_USER=svc-officetool          # AD 里预先建好的机器账号
AUTH_LDAP_PASSWORD=...
```

**`AUTH_DOMAIN` 是必需项，不是可选项。** Linux 上的 Kerberos 认证**不返回任何组信息**
（这是 .NET Negotiate 在 Linux 上的已知行为）——必须显式配 LDAP 查组，
否则域用户能登录进来，但属于 0 个组、因而看不到任何内容，表现为「登录成功但一片空白」。

Negotiate 本身还需要 AD 侧配合（缺一不可）：

1. AD 里建机器账号并注册 SPN：`setspn -S HTTP/nas.你的域.com <机器账号>`
2. `ktpass` 生成 keytab，挂进容器并设 `KRB5_KTNAME`
3. 容器内配 `/etc/krb5.conf`
4. **客户端必须用主机名 FQDN 访问**（`http://nas.你的域.com:8080`），
   用 IP 访问时 SPN 不匹配，SSO 直接失败——这是最容易翻车的一点，需要内网 DNS 或 hosts
5. 时间偏差 < 5 分钟（Kerberos 硬性要求）
6. 浏览器需把该主机视为内网区域，域环境通常由 GPO 下发

**经群晖反向代理时 SSO 基本不可用**：Negotiate 是 HTTP 层握手，反代不透传就会退化成 401。
这种部署请用表单登录（域账号 + 密码走 LDAP 校验），不要指望免登录。

SSO 失败时前端会自动回落到登录页，不会卡死——这是刻意保留的兜底通道。

### 7.4 授权模型

权限挂在**用户组**上，不直接挂人：

- **用户组**：分「域组」（成员由 SSO 同步，界面不允许手工增删成员——手工加的人
  会在该成员下次登录时被整体覆盖冲掉）和「本地组」（成员手工维护）。
  需要手工归拢的人请放进本地组。
- **授权条目**：`组 × 项目 × 检项`。检项留空表示对整个项目生效（含以后新建的检项）。
- **权限等级**（四级递进，高级别自动含低级别）：

  | 等级 | 能做什么 |
  |---|---|
  | 只读 | 看列表与详情、复制路径、打开文件 |
  | 编辑 | ＋上传模板、新建文档、重命名、复制到其他文件夹 |
  | 管理 | ＋删除（进回收站）、回收站恢复、建删检项 |
  | 规则管理 | ＋维护提取规则（上传 / 删除 / 备注 / 复制） |

- 检项级条目**覆盖**项目级条目，因此可以配「整个项目只读 + 某检项可编辑」，
  也可以用「无权限」把某人从项目里单独摘出去。

「用户管理 → 有效权限」能展开某人**实际**能看到哪些检项，以及每一份权限来自哪个组的哪一条。
排查「他为什么看不到这个项目」时从那里看，不要靠猜。

### 7.5 先关掉鉴权把功能跑通

若还没准备好配账号，可以先把鉴权关掉（回到改造前的行为）：
...
> ⚠️ 关闭后**任何能访问该端口的人都是系统管理员**。只在你确认 NAS 仅内网可达、
> 且绝不做公网端口转发时使用。

### 7.6 脚本 / 自动化怎么访问

开启鉴权后冒烟脚本必须带凭据，否则会因为全线 401 而看起来「到处都坏」：

```bash
# 方式一：访问令牌（网页「访问令牌」页生成）
OFFICETOOL_TOKEN=otk_xxx bash scripts/api-smoke.sh

# 方式二：账号口令（脚本内部自动换 Cookie）
OFFICETOOL_USER=admin OFFICETOOL_PASS='...' bash scripts/api-smoke.sh
```
...
1. **鉴权默认开启**。`Auth__Enabled=false` 是完全放行形态，仅限可信内网。
   不确定就保持 `true`，取初始超管密码登录后再授权。
2. 务必限制为内网可达，不要做端口转发暴露到公网。鉴权是内网内的第二道防线，
   不是面向公网的安全边界。
3. 端口绑定可进一步收紧——把 compose 里改成 `"127.0.0.1:8080:8080"` 只允许本机，
   再通过群晖反向代理对外提供（可叠加群晖的访问控制）。
4. 定期备份 `officetool-state` 与 `officetool-keys` 两个卷。
   后者是 Data Protection 密钥环，丢了会导致已加密的凭据无法解密。
5. HTTP 明文传输下面向公网时，登录口令与 Cookie 都会被嗅探。
   内网可用；若要出内网，必须先上 HTTPS（反向代理 + 证书）。
6. 关注镜像基础版本更新（`mcr.microsoft.com/dotnet/aspnet:8.0` 的安全补丁）。
